...
class CiBaselineTests(unittest.TestCase):
    ...
    def test_ci_workflow_matrix_covers_all_platforms(self) -> None:
        ...
        self.assertIn(f'actions/checkout@{actions["checkout_action_ref"]}', workflow)
        self.assertIn(f'dtolnay/rust-toolchain@{actions["rust_toolchain_action_ref"]}', workflow)
        self.assertIn(f'actions/setup-python@{actions["setup_python_action_ref"]}', workflow)
        self.assertIn(f'actions/setup-dotnet@{actions["setup_dotnet_action_ref"]}', workflow)
