...
use simple_si_units::mechanical::{Acceleration, Velocity};
...
impl LCGRand {
    fn rand_f64(&mut self) -> f64 {
        self.seed = (self
            .seed
            .overflowing_mul(0x5DEECE66Du64)
            .0
            .overflowing_add(0xBu64)
            .0)
            & 0xFFFFFFFFFFFFu64;
...
fn print_system(masses: &[MassPoint]) {
    ...
    for mp in masses {
        ...
        if textx > 0 && textx < 20 && texty > 0 && texty < 20 {
            rows[(19 - texty) as usize][textx as usize] = match i {
                0 => '*',
                _ => 'o',
            };
        }
