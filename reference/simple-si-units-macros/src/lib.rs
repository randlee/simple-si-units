...
pub fn derive_unit(tokens: TokenStream) -> TokenStream {
    ...
    let input: syn::DeriveInput = syn::parse(tokens).expect(
        "syn::parse failed on proc macro \
...
fn impl_derive_unit(input: &syn::DeriveInput) -> TokenStream {
    ...
    let fields = match &input.data {
        ...
        _ => panic!(
            "Only structs with named fields can derive simple_si_units::UnitStruct.\n\n{}",
            requirements_msg
        ),
    };
    if fields.len() != 1 {
        panic!(
            "proc macro simple_si_units::UnitStruct can only be used on structs with a single \
		 named field. \n\n{}",
            requirements_msg
        )
    }
