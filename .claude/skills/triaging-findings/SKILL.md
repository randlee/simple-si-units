...
version: 1.1.0
description: Orchestrate pre-dispatch QA finding triage as team-lead. Launch one qa-triage agent per finding, collect phase-scoped Turtle records, aggregate by promoted branch, and only then dispatch branch-scoped fix assignments to cunit.
depends_on:
...
across worktrees before any fix work is sent to `cunit`.
...
2. The target phase has an explicit `phase_id` such as `phase-A`.
...
- `cunit` receives only post-triage, branch-scoped fix assignments.

Do not send raw QA findings directly to `cunit`.
...
artifacts to git before sending any dev assignment to `cunit`.
...
### 4. Dispatch branch-scoped fix work to `cunit`
...
5. send one branch-scoped ATM assignment to `cunit`
...
At the end of a phase, after all sprint branches are integrated into the
phase integration branch (for example `integration/phase-A`) and before the
final merge to `develop`, run the
...
- run one final integration-branch quality gate
...
Required gate:
- `quality-mgr` must run a full review on the phase integration branch
...
- that review team must verify no integrated fix was missed outside the original
  changed-file scopes
- do not merge the phase integration branch to `develop` until that review passes
