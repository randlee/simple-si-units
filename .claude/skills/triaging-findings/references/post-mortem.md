...
- all sprint branches have been integrated into the phase integration branch
- all queued QA findings are resolved or intentionally deferred
- follow-up QA has closed the phase findings set
- a final phase-ending review is run on the phase integration branch before the merge to
  `develop`
...
- fix-dispatch batches sent to `cunit`
- follow-up QA closeout messages from `quality-mgr`
- the final phase integration-branch verification report from `quality-mgr`

## Required Integration Review

Before the final merge to `develop`, `quality-mgr` must run one full review on
the phase integration branch after all sprint integrations are complete.
...
- no finding marked fixed on a sprint branch is still reproducible on the phase
  integration branch
