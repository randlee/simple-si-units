...
version: 0.2.0
description: Procedure for backing up and restoring the units-x team. Referenced by the team-lead skill when a session ID mismatch is detected.
---
...
```bash
atm teams backup unit-x
```

Also back up the Claude Code project task list separately:

```bash
BACKUP_PATH=$(ls -td ~/.claude/teams/.backups/unit-x/*/ | head -1)
cp -r ~/.claude/tasks/unit-x/ "$BACKUP_PATH/tasks-cc"
...
```text
TeamCreate(team_name="unit-x", description="unit-x team", agent_type="team-lead")
```

Verify that the returned team name is exactly `units-x`. If it is not, stop.
...
```bash
atm teams restore unit-x --from ~/.claude/teams/.backups/unit-x/<timestamp>
```
...
python3 -c "
import json
path = '/Users/randlee/.claude/teams/unit-x/config.json'
with open(path) as f:
    cfg = json.load(f)
keep = ['team-lead', 'cunit', 'quality-mgr']
...
if [ -d "$BACKUP_PATH/tasks-cc" ]; then
  mkdir -p ~/.claude/tasks/unit-x
  cp "$BACKUP_PATH/tasks-cc/"*.json ~/.claude/tasks/unit-x/ 2>/dev/null || true
  MAX_ID=$(ls ~/.claude/tasks/unit-x/*.json 2>/dev/null \
    | xargs -I{} basename {} .json \
    | sort -n | tail -1)
  [ -n "$MAX_ID" ] && echo -n "$MAX_ID" > ~/.claude/tasks/unit-x/.highwatermark
...
```bash
atm send cunit "New session (session-id: <SESSION_ID>). Team unit-x restored. Please acknowledge and confirm status."
```
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `TeamCreate` returns random name | `~/.claude/teams/unit-x` still exists | remove the directory and retry |
