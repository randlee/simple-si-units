...
```bash
python3 -c "import json; print(json.load(open('/Users/randlee/.claude/teams/unit-x/config.json'))['leadSessionId'])"
```
...
| `/codex-orchestration` | Run phases where cunit is sole dev, with pipelined QA via quality-mgr |
...
| `/triaging-findings` | Correlate QA findings across branches before dispatching fixes to cunit |
...
- Codex agents such as `cunit` only see new ATM messages when they check
