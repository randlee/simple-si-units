# Step 1 — Plan Scope Review (`cunit`)
...
```bash
atm send cunit --stdin < /tmp/step-1-message.xml
```
...
- `FAIL` -> re-render and re-send Step 1 to `cunit`
- if `cunit` ACKs but returns no new fenced JSON, increment `round_index`,
