...
description: >
  ...
  units-x still exists on disk and the saved leadSessionId still matches the
...
- `SESSION_ID` still matches `leadSessionId` in
  `~/.claude/teams/unit-x/config.json`
...
```text
SendMessage(to="<claude-teammate>", message="ping: verify units-x communications path")
```
...
```bash
atm teams restore unit-x --from "$BACKUP_PATH"
```
