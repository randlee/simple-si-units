...
version: 0.1.0
description: Orchestrate multi-sprint phase execution as team-lead. Manages sprint waves, scrum-master lifecycle, PR merges, cunit reviews, and integration branch strategy. This skill is for team-lead only, not for scrum-masters.
depends_on:
  scrum-master: 0.x
  rust-developer: 0.x
  rust-qa-agent: 0.x
...
4. `cunit` is running and reachable via ATM CLI.
...
1. before QA-1, require `cunit` to run a self-directed Rust best-practices
...
### 4. Post-sprint: cunit design review

After every sprint PR is merged to `integrate/phase-{N}`, request a `cunit`
review via ATM CLI. Do not block the next eligible sprint unless cunit
reports critical blocking findings.

### 5. Fix sprint if needed

If cunit finds issues:
1. create a new worktree from `integrate/phase-{N}`
2. let cunit or a fresh scrum-master execute the fixes
...
3. critical cunit findings must be addressed first
...
- cunit is persistent and communicates via ATM CLI

## ATM CLI Communication

Use ATM CLI for cunit:

```bash
atm send cunit "message"
atm read
atm inbox
```
...
- do not skip post-merge cunit reviews
