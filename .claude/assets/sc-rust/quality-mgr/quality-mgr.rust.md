...
- launch `rust-service-hardening-agent` in `sprint_review` mode only on an explicit override or when the changed scope is actually service-like; it is not part of the standing `units-x` QA set
